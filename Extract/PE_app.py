...
with open('/Users/krishna/Desktop/RAIDEN.zip (Unzipped Files)/Classifier/PE/pickel_malware_detector.pkl', 'rb') as file:
    detector = pickle.load(file)
file.close()
with open("/Users/krishna/Desktop/RAIDEN.zip (Unzipped Files)/Classifier/PE/features.pkl", 'rb') as file2:
...
classifier = tf.keras.saving.load_model(
    filepath='D:\\Viren\\AIKD\\Malware\\malware-dection\\Classifier\\Malware_Classifier\\tf_save_model.keras',
    custom_objects={
        'f1_m': f1_m,
...
class PE_scanner:
    ...
    def PE_scan(self, fpath: str) -> int:
        ...
        if data != {}:
            ...
            pe_result = self.detector.predict([pe_features])
            print(pe_result)
    ...
    def PE_mal_classify(self, fpath: str) -> str:
        ...
        return class_names[np.argmax(score)],data
